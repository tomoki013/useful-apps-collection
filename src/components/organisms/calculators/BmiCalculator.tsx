...
export const BmiCalculator = () => {
  ...
  return (
    <Card className="w-full max-w-md mx-auto">
      <CardHeader>
        <CardTitle>{t('bmi-calculator.title')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Unit System Selection */}
        <div className="space-y-2">
          <Label>{t('bmi-calculator.unit')}</Label>
          <RadioGroup value={formState.unit} onValueChange={handleUnitChange} className="flex space-x-4">
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="metric" id="metric" />
              <Label htmlFor="metric">{t('bmi-calculator.metric')}</Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="imperial" id="imperial" />
              <Label htmlFor="imperial">{t('bmi-calculator.imperial')}</Label>
            </div>
          </RadioGroup>
        </div>
        ...
        {formState.unit === 'metric' ? (
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="heightCm">{t('bmi-calculator.heightCm')}</Label>
              <Input id="heightCm" type="number" value={formState.heightCm} onChange={handleInputChange} placeholder="例: 170" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="weightKg">{t('bmi-calculator.weightKg')}</Label>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>{t('bmi-calculator.height')}</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="weightLbs">{t('bmi-calculator.weightLbs')}</Label>
      ...
      <CardFooter className="flex flex-col items-stretch">
        <Button onClick={handleCalculate}>{t('bmi-calculator.calculate')}</Button>
        ...
        <AnimatePresence>
          {result && (
            ...
            >
              <h3 className="text-lg font-semibold mb-2">{t('bmi-calculator.resultTitle')}</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>{t('bmi-calculator.bmi')}:</span>
                  <span className="font-bold">{result.bmi}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t('bmi-calculator.category')}:</span>
                  <span className="font-bold">{t(`bmi-calculator.categories.${result.category}`)}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t('bmi-calculator.idealWeight')}:</span>
